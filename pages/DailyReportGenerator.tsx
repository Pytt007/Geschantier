...
export const DailyReportGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const projectName = project ? project.name : 'Projet';
      
      const dateContext = startDate === endDate 
        ? startDate 
        : `du ${startDate} au ${endDate}`;

      const promptContext = `Contexte du rapport :
- Projet : ${projectName}
- Période : ${dateContext}
- Type de rapport : ${reportType}

Points clés renseignés par l'utilisateur :
${points}`;

      const result = await generateDailyReport(promptContext);
      setReport(result);
    } catch (error) {
    ...
    <div className="space-y-8">
      ...
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button 
            ...
            className="flex items-center justify-center h-11 px-6 bg-indigo-600 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
           {isLoading ? <Spinner /> : (
               <>
                    ...
                    <div>
                     ...
                     <div>
                        <div className="flex justify-between items-center pb-2">
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Points clés (IA)</label>
                        </div>
                        <textarea 
                            rows={6}
                            value={points}
                            onChange={(e) => setPoints(e.target.value)}
                            placeholder="- Coulage dalle terminé&#10;- Météo favorable&#10;- Réunion sécurité OK"
            ...
            <div className="p-6 bg-white dark:bg-card-dark rounded-xl shadow-sm border border-slate-100 dark:border-slate-700/50 min-h-[600px] flex flex-col">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-700 pb-4 mb-6">
                    ...
                    <div className="flex items-center gap-2">
                        <button disabled={!report} className="flex items-center justify-center h-10 px-4 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                     ...
                     {report ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50/50 dark:bg-slate-800/30 p-8">
                            ...
                            <p className="text-sm max-w-sm text-center mt-2">
                                Remplissez les points clés à gauche et cliquez sur "Générer" pour créer votre rapport.
                            </p>
                        </div>
                     )}
                </div>
