...
export const ProjectFormModal: React.FC<ProjectFormModalProps> = ({ isOpen, onClose, onSubmit, projectToEdit, teamMembers }) => {
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={isEditing ? 'Modifier le projet' : 'Créer un nouveau projet'}
            maxWidth="2xl"
        >
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    <div className="flex items-center justify-center w-full">
                        <label htmlFor="featured-image-upload" className="flex flex-col items-center justify-center w-full h-40 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 dark:bg-slate-800/50 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors overflow-hidden relative">
                    ...
                    {formData.featuredImage && (
                        <div className="flex justify-end mt-2">
                            <button type="button" onClick={() => setFormData(prev => ({ ...prev, featuredImage: '' }))} className="text-xs font-medium text-red-600 hover:text-red-800 dark:text-red-400 transition-colors">Supprimer l'image</button>
                        </div>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nom du projet</label>
                    ...
                    <div>
                        <label htmlFor="manager" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Chef de projet</label>
                        <select name="manager" id="manager" value={formData.manager} onChange={handleChange} required className="block w-full bg-white dark:bg-slate-800 dark:border-slate-600 dark:text-white border border-slate-300 rounded-lg shadow-sm p-2.5 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
                ...
                <div>
                    <label htmlFor="address" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Adresse du chantier</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label htmlFor="budget" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Budget (FCFA)</label>
                        <input type="number" name="budget" id="budget" value={formData.budget || ''} onChange={handleChange} required min="1" className="block w-full bg-white dark:bg-slate-800 dark:border-slate-600 dark:text-white border border-slate-300 rounded-lg shadow-sm p-2.5 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
                    </div>
                    <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label htmlFor="endDate" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Date de fin</label>
                        <div className="relative">
                ...
                <div className="flex justify-end pt-6 space-x-3">
                    ...
                    <button type="submit" disabled={!isFormValid} className="px-5 py-2.5 rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 font-semibold disabled:bg-slate-400 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/20 transition-all hover:shadow-indigo-500/40">{isEditing ? 'Sauvegarder' : 'Créer le projet'}</button>
                </div>
            </form>
        </Modal>
    );
